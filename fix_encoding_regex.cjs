...
text = text.replace(/alert\('계좌번호가 복사.*?\\n' \+ text\);/, "alert('계좌번호가 복사되었습니다.\\n' + text);");
...
text = text.replace(/\/\/ BGM.*?태.*?디.*?어/, "// BGM 상태 및 오디오 제어");
...
text = text.replace(/\{\/\* 방명.*?리스.*?최.*?3개까지.*?출\) \*\/\}/, "{/* 방명록 리스트 (최대 3개까지만 노출) */}");
...
text = text.replace(/alert\(`\[카카.*?톡 공유.*?\].*?목: \$\{shareInfo\.title\}\\n.*?명: \$\{shareInfo\.description\}\\n\\n\(.*?제.*?경.*?서.*?카카.*?톡.*?이.*?립.*?다\.\)`\)/, "alert(`[카카오톡 공유 썸네일 확인]\\n\\n제목: ${shareInfo.title}\\n설명: ${shareInfo.description}\\n\\n(실제 환경에서는 카카오톡 앱이 열립니다.)`)");
text = text.replace(/카카.*?톡.*?로 공유.*?기/, "카카오톡으로 공유하기");
text = text.replace(/alert\('.*?링크가 복사.*?었.*?니.*?\\n.*?하.*?에 붙여.*?기\(Ctrl\+V\).*?세.*?'\);/, "alert('초대장 링크가 복사되었습니다.\\n원하시는 곳에 붙여넣기(Ctrl+V) 하세요.');");
...
text = text.replace(/return alert\('.*?모두.*?력.*?주.*?요\.'\);/, "return alert('모든 항목을 입력해주세요.');");
text = text.replace(/return alert\('비.*?번.*?자 4.*?리.*?력.*?주.*?요\.'\);/, "return alert('비밀번호는 숫자 4자리로 입력해주세요.');");
text = text.replace(/alert\('방명록이.*?록.*?었.*?니.*?'\);/, "alert('방명록이 등록되었습니다.');");
text = text.replace(/<button[^>]*>\s*.*?록.*?기\s*<\/button>/g, match => match.replace(/.*?록.*?기/, '등록하기'));
text = text.replace(/\{\/\* Guestbook List Modal \(.*?체보기.*? \*\/\}/, "{/* Guestbook List Modal (전체보기 팝업) */}");

fs.writeFileSync('src/components/preview/InvitationPreview.jsx', text, 'utf8');
console.log('Regex fix complete');
